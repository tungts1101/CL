...
class AlignmentLearner(BaseMergingLearner):
    ...
    def post_training(self):
        ...
        if train_ca_method in ["sgd", "nes"] and False:
            logging.info(
    ...
    def _align_sgd(self):
        ...
        if self._buffer is not None:
            logging.info(
                f"[Alignment] Using buffer with size {self._buffer.size} for SGD alignment"
            )
            for epoch in range(epochs):
                total_loss, total, total_acc = 0, 0, 0
                for x, z, y in self._buffer.__iter__(
                    batch_size=batch_size, seed=epoch * 1000
                ):
                    # x, y = x.cuda(), y.cuda()
                    # with torch.no_grad():
                    ...
                    z, y = z.cuda(), y.cuda()
                    logits = self._network.fc(z)["logits"]
            ...
            logging.info(f"[Alignment] Using Gaussian sampling for SGD alignment")
            for epoch in range(epochs):
    ...
    def _align_nes(self):
        if self._buffer is not None:
            ...
            def get_accuracy():
                total_acc = 0
                for _, z, y in self._buffer.__iter__(batch_size=512):
                    z, y = z.cuda(), y.cuda()
                    logits = self._network.fc(z)["logits"]
                    total_acc += (logits.argmax(dim=1) == y).sum().item()
                return total_acc / self._buffer.size
            ...
            def get_accuracy():
                ...
                for i in range(0, len(sampled_data), 512):
                    ...
                    total_acc += (
                        (logits.argmax(dim=1) == sampled_label[i : i + 512])
                        .sum()
                        .item()
                    )
        ...
        for t in range(self._cur_task + 1):
            lam = lambda_cfg.get(
                str(t),
                lambda_cfg.get(t, default_cur if t == self._cur_task else default_old),
            )
        ...
        def get_sigma_vec(iteration, total_iters):
            decay_factor = (base_sigma_final / base_sigma_init) ** (
        ...
        for it in range(iters):
            ...
            if it % patience == 0 or it == iters - 1:
                ...
                logging.info(
                    f"[Alignment][NES-diag] iter {it}: best_loss={best_loss:.6f}, acc={acc:.4f}, sigma_range={current_sigma_range}"
                )

            if no_improve_steps >= patience:
                logging.info(
                    f"[Alignment][NES-diag] Early stop at iter {it} (no improvement for {patience})."
                )
                break
    ...
    def _align_projection(self):
        ...
        def _auto_lambda_simple(X, Y):
            ...
            lambdas = (
                torch.tensor(10.0, device=device) ** torch.arange(-3, 6, device=device)
            ).float()
        ...
        if not hasattr(self, "_proxy_loader"):
            logging.info(
                "[Alignment] Creating proxy data loader for feature collection"
            )
            proxy_data_manager = DataManager(
                "tinyimagenet", True, self.args["seed"], 200, 0, self.args
            )
            proxy_set = proxy_data_manager.get_dataset(
                np.arange(0, 200), source="train", mode="test"
            )
            proxy_subset = self.get_subset_per_class(
                proxy_set, percent=0.1, seed=self.args.get("seed", 42)
            )
            self._proxy_loader = DataLoader(
                proxy_subset, batch_size=512, shuffle=False, num_workers=4
            )
        ...
        if self.args.get("train_merge_incremental", False):
            ...
            prev_backbone.load_state_dict(
                torch.load(self.merge_checkpoint(self._cur_task - 1)), strict=False
            )
            curr_backbone.load_state_dict(
            ...
            if b_prev is not None:
                b_prev = b_prev.float().contiguous()
            if b_curr is not None:
                b_curr = b_curr.float().contiguous()
            ...
            for i in range(self._cur_task):
                ...
                if i < self._cur_task:
                    ...
                    b_block = (
                        None
                        if b_prev_rot is None
                        else b_prev_rot[offset_prev : offset_prev + C_slice]
                    )
                    offset_prev += C_slice
                else:
        ...
        def colwise_cos(W_new, W_old, reduce="mean", eps=1e-12):
            ...
            cos = (a * b).sum(0)  # (C,)
            return (
                cos.mean().item() if reduce == "mean" else cos
            )  # return vector if reduce=None
